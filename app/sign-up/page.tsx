...
import { AtSign, Eye, EyeOff, Lock, Loader2, User } from "lucide-react";
        ...
        <section className="flex items-center justify-center px-6 py-10 md:px-10 lg:py-12">
          <div className="w-full max-w-[460px] rounded-2xl bg-white px-10 py-10 shadow-[0_4px_24px_rgba(0,0,0,0.08)] md:px-12">
            ...
            {registerMutation.isError ? (
              <div className="mt-4">
                <ErrorMessage message={getErrorMessage(registerMutation.error)} />
              </div>
            ) : null}
            <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
              <div>
                ...
                <div className="relative">
                  ...
                  <input id="fullName" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your Full Name" className="mv-input h-12 pl-11" required />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="mv-input h-12 pl-11" required />
                </div>
              </div>

              <div>
                <label htmlFor="phone" className="mb-2 block text-[11px] font-bold uppercase tracking-[0.14em] text-[#1565c0]">
                  Phone (optional)
                </label>
                <input id="phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="01XXXXXXXXX" className="mv-input h-12" />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                  />
                  <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#9CA3AF]" aria-label={showPassword ? "Hide password" : "Show password"}>
                ...
                {strength && (
                  <div className="mt-2">
                    <div className="h-1.5 w-full overflow-hidden rounded-full bg-[#E5E7EB]">
                      <div className="h-full rounded-full transition-all duration-300" style={{ width: strengthConfig[strength].width, backgroundColor: strengthConfig[strength].color }} />
                    </div>
                    <p className={cn("mt-1 text-[10px] font-semibold uppercase tracking-wide", { "text-[#ef4444]": strength === "weak", "text-[#eab308]": strength === "medium", "text-[#22c55e]": strength === "strong" })}>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                  />
                  <button type="button" onClick={() => setShowConfirmPassword((v) => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#9CA3AF]" aria-label={showConfirmPassword ? "Hide confirm password" : "Show confirm password"}>
              ...
              <button type="submit" disabled={registerMutation.isPending || password !== confirmPassword} className="flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#1A2E6F] text-[14px] font-bold text-white transition hover:bg-[#0d47a1] disabled:opacity-60">
